# Guides

- [Install the BeeApiary App](install-app.md)
- [Set Up the BeeApiary Beehive Scales](set-up-device.md)
- [Configure GSM](configure-gsm.md)
- [Connect to the Device Access Point](configure-local-wifi.md)
- [Set Up Synchronization Through the Apiary Wi-Fi Network](configure-wifi-sync.md)
- [Verify Wi-Fi Synchronization Settings](verify-wifi-sync-settings.md)
- [Set Up Bluetooth Synchronization](configure-bluetooth-sync.md)
- [Download the Annual Archive](download-archive.md)
- [Tare the Scales](tare-scales.md)
- [Calibrate the Scales](calibrate-scales.md)
- [Update the Device Firmware](update-device.md)
- [Update the App](update-app.md)
